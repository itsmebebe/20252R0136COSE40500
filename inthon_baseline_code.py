# -*- coding: utf-8 -*-
"""InThon Baseline Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1M8XTtUwwaQWuLe032px5x8sEPK9u9-g-
...
# BitsAndBytesConfig 설정 (8비트 양자화 사용)
...
lora_config = LoraConfig(
    ...
    lora_dropout=0.1,
    bias="none",
    task_type="SEQ_2_SEQ_LM",  # 필요한 경우 "CAUSAL_LM"으로 설정 가능
)
...
class ImageCaptionDataset(Dataset):
    def __init__(self, df, processor, model):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            processor_inputs = self.processor(images=image, text=caption, return_tensors="pt")
        except Exception as e:
            print(f"Error processing image at {image_url}: {e}")
            return None

        # BLIP-2 모델을 사용하여 임베딩 및 특징 추출
        with torch.no_grad():
            outputs = self.model(
            ...
            text_embeds = outputs.language_model_outputs.last_hidden_state[:, 0, :]
            image_features = outputs.qformer_outputs.last_hidden_state
            text_features = outputs.language_model_outputs.last_hidden_state

        processor_inputs = {key: tensor.squeeze(0) for key, tensor in processor_inputs.items()}
        labels = processor_inputs['input_ids']
...
def custom_data_collator(features):
    ...
    image_embedding = [feature['image_embedding'] for feature in features]
    text_embedding = [feature['text_embedding'] for feature in features]

    max_len_labels = max(len(label) for label in labels)
    max_len_image_embedding = max(len(emb) for emb in image_embedding)
    max_len_text_embedding = max(len(emb) for emb in text_embedding)

    padded_labels = [torch.cat([label, torch.zeros(max_len_labels - len(label), dtype=label.dtype)], dim=0) for label in labels]
    padded_image_embedding = [torch.cat([emb, torch.zeros(max_len_image_embedding - len(emb), dtype=emb.dtype)], dim=0) for emb in image_embedding]
    padded_text_embedding = [torch.cat([emb, torch.zeros(max_len_text_embedding - len(emb), dtype=emb.dtype)], dim=0) for emb in text_embedding]

    batch = {
        'pixel_values': torch.stack(pixel_values),
        'labels': torch.stack(padded_labels),
        'image_embedding': torch.stack(padded_image_embedding),
        'text_embedding': torch.stack(padded_text_embedding),
    }
    return batch

import torch
from torch.utils.data import DataLoader
train_dataset = ImageCaptionDataset(train_df, processor, model)
val_dataset = ImageCaptionDataset(val_df, processor, model)
test_dataset = ImageCaptionDataset(test_df, processor, model)

train_dataloader = DataLoader(
    train_dataset,
    batch_size=8,
    shuffle=True,
    collate_fn=custom_data_collator  # Use the custom collate function
)
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_dataloader, desc=f"Training Epoch {epoch+1}"):
        ...
        # 손실 계산
        total_loss, itc_loss, itm_loss = loss_fn(image_embeds, text_embeds, image_features, text_features)
        total_loss.backward()
    ...
    with torch.no_grad():
        for batch in tqdm(val_dataloader, desc=f"Validation Epoch {epoch+1}"):
            ...
            total_loss, _, _ = loss_fn(image_embeds, text_embeds, image_features, text_features)
...
class BLIP2Loss(nn.Module):
    def __init__(self, temperature=0.07):
        super().__init__()
        self.temperature = temperature

    def forward(self, image_embeds, text_embeds, image_features, text_features):
        itc_loss = self.image_text_contrastive_loss(image_embeds, text_embeds)
        itm_loss = self.image_text_matching_loss(image_features, text_features)
        total_loss = itc_loss + itm_loss
        return total_loss, itc_loss, itm_loss

    def image_text_contrastive_loss(self, image_embeds, text_embeds):
    ...
    def image_text_matching_loss(self, image_features, text_features):
        joint_features = torch.cat([image_features, text_features], dim=-1)
        logits = nn.Linear(joint_features.shape[-1], 2)(joint_features)
...
# 기존 모델의 inputs_embeds 제거를 위한 custom forward 메서드
def new_forward(self, *args, **kwargs):
    if 'inputs_embeds' in kwargs:
        kwargs.pop('inputs_embeds')
    return self.base_model.forward(*args, **kwargs)

# 새로운 forward 메서드를 모델에 바인딩
...
def forward_pass(pixel_values, labels, image_embedding, text_embedding):
    # 모델의 기본적인 forward pass
    outputs = model(
    ...
    cross_entropy_loss = criterion(logits.view(-1, logits.size(-1)), labels.view(-1))

    # Cosine 유사도 손실
    similarity_loss = 1 - F.cosine_similarity(image_embedding, text_embedding).mean()

    # 최종 손실 계산
    total_loss = cross_entropy_loss + 0.5 * similarity_loss
    return total_loss
...
for epoch in range(num_epochs):
    ...
    with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch + 1} (Training)", postfix={'Train Loss': 0.0}) as pbar:
        for idx, batch in enumerate(train_dataloader):
            ...
            # 손실 계산
            loss = forward_pass(pixel_values, labels, image_embedding, text_embedding)
            ...
            # 진행 상황 업데이트
            avg_train_loss = epoch_loss / (idx + 1)
    ...
    with torch.no_grad():
        with tqdm(total=len(val_dataloader), desc=f"Epoch {epoch + 1} (Validation)", postfix={'Val Loss': 0.0}) as pbar:
            for idx, batch in enumerate(val_dataloader):
                ...
                pbar.set_postfix({'Val Loss': avg_val_loss})
                pbar.update(1)
